...
import { IFormInputsClient } from '@types';
...
const EditClients = () => {
  ...
  const { loading: isClientDataLoading, data: getClientData } = useQuery(GET_CLIENT_ID, {
    fetchPolicy: 'network-only',
    variables: {
      clientId: clientId,
    },
  });
  const { loading, data: listUsers } = useQuery(LIST_USERS, {
    fetchPolicy: 'network-only',
  });
  const { loading: officeloading, data: AllOffices } = useQuery(ALL_OFFICES);
  ...
  useEffect(() => {
    if (!loading && !isClientDataLoading) {
      const data = clientData?.mortgageProperty?.map((item: any) => {
        return item;
      });

      const additionalRecord = data?.map((ele: any) => ele?.additionalProducts);
      ...
      reset({
        ...
        fullName_solicitor: clientData?.solicitorDetails?.fullName,
        email_solicitor: clientData?.solicitorDetails?.email,
        phone_solicitor: clientData?.solicitorDetails?.phoneNumber,
        ...
        amount: parseInt(data?.[0]?.amountRequired),
        lender: data?.[0]?.lender,
        next_renewal_date: data?.[0]?.nextRenewalDate,
      });
    }
  }, [reset, clientData, loading, isClientDataLoading]);

  return (
    <>
      <Box p={4}>
        {isClientDataLoading && loading ? (
          <Loader isFullScreen />
        ) : (
            ...
            <Box sx={{ paddingY: '15px' }}>
              ...
              <form>
                <Box
                  ...
                  <Box
                    sx={{ fontSize: '24px', color: '#2B388C', fontWeight: '400', marginY: '20px' }}>
                    {clientData?.fullName}
                  </Box>
                  <Button
                    variant="contained"
                    ...
                    <Box sx={{ width: '50%' }}>
                          ...
                          <FormControl
                            ...
                            <Controller
                              ...
                              render={({ field: { onChange, value } }: any) => (
                                <Select
                                  name="advisor"
                                  defaultValue={clientData?.adviser}
                                  sx={{
                                    height: '40px',
                                    background: 'rgba(224, 231, 255, 0.2)',
                                    border: '2px solid #E0E7FF',
                            ...
                            <Controller
                              ...
                              render={({ field: { onChange, value } }: any) => (
                                <Select
                                  ...
                                  defaultValue={clientData?.officeId}
                                  onChange={onChange}
                                  value={value}>
                                  {!officeloading &&
                                    allOffices?.length &&
                      ...
                      <Box sx={{ marginTop: '20px', display: 'flex', flexDirection: 'column' }}>
                        ...
                        <OutlinedInput
                          ...
                          {...register('rerpayment_method', {
                            required: 'Rerpayment Method is required',
                          })}
                          sx={{
                            height: '40px',
                            background: 'rgba(224, 231, 255, 0.2)',
